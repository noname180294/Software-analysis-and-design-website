    ...
    <>
      ...
      <div className="p-6 max-w-5xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {posts.map((post) => (
            <div key={post.id} className="p-4 bg-white rounded-lg shadow-lg border">
              <h2 className="text-xl font-semibold text-pink-500">{post.title}</h2>
              <p className="text-gray-500 flex items-center mt-1">
                <FaCalendarAlt className="mr-2 text-pink-500" /> {post.date}
              </p>
              <p className="text-gray-500 flex items-center mt-1">
                <FaUser className="mr-2 text-pink-500" /> {post.author}
              </p>
              <p className="text-gray-500 flex items-center mt-1">
                <FaTag className="mr-2 text-pink-500" /> {post.category}
              </p>
              <p className="text-gray-500 flex items-center mt-1">
                <FaMapMarkerAlt className="mr-2 text-pink-500" /> {post.location}
              </p>
              <p className="mt-2 text-pink-500">{post.content}</p>
